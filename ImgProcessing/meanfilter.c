/* meanfilter.c, to be applied to a PPM format image with a desired size for the mean filter */
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
#include <unistd.h>
...
void mean_color(rgb_t *pix, const color_histo_t *h)
{
    pix->r = mean(h->r, h->n);
    pix->g = mean(h->g, h->n);
    pix->b = mean(h->b, h->n);
...
image mean_filter(image in, int size) //mean filter main function, returns the new image to be written in current directory (using write_ppm())
{
    int row, col;
