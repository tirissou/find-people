...
class ConvolutionLayer
{
 public:
  ...
  void Forward(arma::cube& input, arma::cube& output)
  {
    ...
    // Print output.
    std::cout << DEBUG_PREFIX << std::endl;
    std::cout <<  "Output of conv layer:" << std::endl;
    for (size_t i=0; i<numFilters; i++)
    {
      std::cout <<  "Output slice #" << i << std::endl;
      for (size_t r=0; r<output.n_rows; r++)
        std::cout <<  output.slice(i).row(r);
    }
#endif
  }
...
#if DEBUG
    std::cout <<  std::endl;
    std::cout <<  "Gradient wrt filters:" << std::endl;
    for (size_t i=0; i<numFilters; i++)
    {
      for (size_t s=0; s < gradFilters[i].n_slices; s++)
      {
        std::cout <<  "Gradient slice #" << s << std::endl;
        for (size_t r=0; r < gradFilters[i].n_rows; r++)
          std::cout <<  gradFilters[i].slice(s).row(r);
      }
    }
#endif
