...
{
 public:
  PeopleData(std::string Dir, double splitRatio = 0.9)
  {
    ...
    int numSamples = trainDataRaw.n_rows;
    ...
    trainData = std::vector<arma::cube>(trainDataAll.begin(),
                                        trainDataAll.begin() + numSamples*splitRatio);
    trainLabels = std::vector<arma::vec>(trainLabelsAll.begin(),
                                         trainLabelsAll.begin() + numSamples*splitRatio);

    validationData = std::vector<arma::cube>(trainDataAll.begin() + numSamples*splitRatio,
                                             trainDataAll.end());
    validationLabels = std::vector<arma::vec>(trainLabelsAll.begin() + numSamples*splitRatio,
                                              trainLabelsAll.end());
